...
module API
  ...
  class Account
    def self.get(account_id: nil, account_title: nil, account_name: nil, account_type: nil)
  ...
  class Accounts
    def self.get(account_ids: [])
      ...
      if account_ids.empty?
        Logger.log(ERROR, 'Account IDs are required')
        return nil
      end
      Logger.log(DEBUG, 'Fetching accounts from Securden')
      ...
      Logger.log(DEBUG, accounts['message']) if accounts['message']
      accounts
    rescue StandardError => e
